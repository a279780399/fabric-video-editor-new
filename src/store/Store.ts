...
export class Store {
  ...
  addVideo(index: number) {
    ...
    const durationLimit = 60 * 1000;

    // Calculate the new dimensions based on the aspect ratio and canvas size
    const aspectRatio = videoWidth / videoHeight;
    let newWidth = canvasWidth;
    let newHeight = canvasWidth / aspectRatio;

    if (newHeight > canvasHeight) {
      // If the calculated height is greater than the canvas height, scale down
      newHeight = canvasHeight;
      newWidth = canvasHeight * aspectRatio;
    }

    // Calculate the scaled center position
    const scaledCenterX = (canvasWidth - newWidth) / 2;
    const scaledCenterY = (canvasHeight - newHeight) / 2;
    ...
    this.addEditorElement({
      ...
      timeFrame: {
        start: 0,
        end: Math.min(durationLimit, videoDurationMs),
      },
      properties: {
        elementId: `video-${id}`,
